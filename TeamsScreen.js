...
import TeamStatisticsScreen from "./TeamStatisticsScreen";
...
const TeamsScreen = () => {
  ...
  const [eventSeasonPickerVisible, setEventSeasonPickerVisible] =
    useState(false);
  ...
  const loadTeamsByLeague = async (leagueName) => {
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        const teamsWithFavorites = teams
          .slice(startIndex, startIndex + teamsPerPage)
          .map((team) => ({
            ...team,
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        const startIndex = (i - 1) * teamsPerPage;
        searchCache[i] = resultsWithFavorites.slice(
          startIndex,
  ...
  const handleSelectTeam = async (team) => {
    ...
    // Limpiar estados de todas las vistas al cambiar de equipo
    setTeamEvents([]);
    setEventsView(false);
    setLineupView(false);
    setStatisticsView(false); // AGREGAR ESTA LÍNEA
  ...
  const renderLeagueItem = ({ item }) => (
    ...
    >
      ...
      <Text
        style={[styles.leagueName, { color: isDarkMode ? "#fff" : "#000" }]}
      >
        {item.league}
      </Text>
    </TouchableOpacity>
  ...
  const renderTeamItem = ({ item }) => (
    ...
    >
      <Image
        ...
        resizeMode="contain"
      />
      <Text style={[styles.teamName, { color: isDarkMode ? "#fff" : "#000" }]}>
        {item.strTeam}
      </Text>

      {item.strLeague && (
        <Text
          style={[styles.teamLeague, { color: isDarkMode ? "#ccc" : "#666" }]}
        >
          {item.strLeague}
        </Text>
      )}
  ...
  const renderSeasonEquipment = (jerseyItemBgColor) => {
    ...
    const seasonData = teamEquipment.find(
    ...
    return (
      <>
        {seasonData.equipment.map((jersey) => (
          <View
            key={jersey.id}
            style={[styles.jerseyItem, { backgroundColor: jerseyItemBgColor }]}
          >
            <Image
  ...
  const renderTeamDetails = () => (
    ...
    >
      ...
      <Text
        style={[styles.teamDetailName, { color: isDarkMode ? "#fff" : "#000" }]}
      >
      ...
      <View style={styles.teamDetailActions}>
        {/* BOTÓN Ver eventos */}
        <TouchableOpacity
          style={[styles.teamDetailAction, { backgroundColor: "#e67e22" }]}
          onPress={() => {
        ...
        >
          <Text style={styles.teamDetailActionText}>{t("viewEvents")}</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.teamDetailAction, { backgroundColor: "#666" }]}
          onPress={() => {
        ...
        >
          <Text style={styles.teamDetailActionText}>{t("viewJerseys")}</Text>
        </TouchableOpacity>

        {/* NUEVO BOTÓN Ver Estadísticas */}
        <TouchableOpacity
          style={[styles.teamDetailAction, { backgroundColor: "#27ae60" }]}
          onPress={() => {
            // Navegar a la nueva pantalla de estadísticas
            setStatisticsView(true);
          }}
        >
          <Text style={styles.teamDetailActionText}>{t("viewStatistics")}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.teamDetailInfo}>
        ...
        {selectedTeam.strStadium && (
          <View style={styles.detailRow}>
            ...
            >
              {selectedTeam.strStadium}
              {selectedTeam.intStadiumCapacity &&
                ` (${selectedTeam.intStadiumCapacity})`}
            </Text>
          </View>
        )}
      ...
      {(selectedTeam.strDescriptionEN ||
        ...
        selectedTeam.strDescriptionIT) && (
        <View style={styles.teamDetailBio}>
          <Text
            style={[styles.bioTitle, { color: isDarkMode ? "#fff" : "#000" }]}
          >
            {t("description")}
          </Text>
          <Text
            style={[styles.bioText, { color: isDarkMode ? "#ccc" : "#333" }]}
          >
  ...
  const renderTeamEvents = () => {
    ...
    const eventItemBackgroundColor = isDarkMode
      ? "rgba(50,50,50,0.6)"
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.eventItem,
                      {
                        backgroundColor: eventItemBackgroundColor,
                  ...
                  >
                    ...
                    {isPastEvent &&
                      ...
                      event.intAwayScore !== null && (
                        <Text
                          style={[styles.eventResult, { color: "#e67e22" }]}
                        >
                          {t("result")}: {event.intHomeScore} -{" "}
                          {event.intAwayScore}
                        </Text>
                      )}
  ...
  const renderEventLineup = () => {
    ...
    const backgroundColor = isDarkMode ? "#333" : "#f5f5f5";
    const headerBackgroundColor = isDarkMode ? "#222" : "#e0e0e0";
    const lineupCardBackgroundColor = isDarkMode
      ? "rgba(50,50,50,0.6)"
      : "rgba(220,220,220,0.6)";
        ...
        {players.length > 0 ? (
          players.map((player, index) => (
            ...
            >
              ...
              >
                {language === "es"
                  ? getPositionInSpanish(player.position)
                  : player.position}
              </Text>
            </View>
          ))
    ...
    return (
      <View style={[styles.container, { backgroundColor: backgroundColor }]}>
        ...
        {selectedEvent &&
          isEventInPast(selectedEvent.dateEvent, selectedEvent.strTime) &&
          selectedEvent.intHomeScore !== null &&
          selectedEvent.intAwayScore !== null && (
            <Text style={[styles.lineupResult, { color: "#e67e22" }]}>
              {selectedEvent.strHomeTeam} {selectedEvent.intHomeScore} -{" "}
              {selectedEvent.intAwayScore} {selectedEvent.strAwayTeam}
            </Text>
          )}
                ...
                {eventLineup.homeTeam.starters && (
                  <>
                    ...
                    {eventLineup.homeTeam.substitutes.length > 0 &&
                      renderPlayersList(
                        eventLineup.homeTeam.substitutes,
                        t("substitutes")
                      )}
                  </>
                )}
                ...
                {eventLineup.awayTeam.starters && (
                  <>
                    ...
                    {eventLineup.awayTeam.substitutes.length > 0 &&
                      renderPlayersList(
                        eventLineup.awayTeam.substitutes,
                        t("substitutes")
                      )}
                  </>
                )}
  ...
  const renderTeamJerseys = () => {
    ...
    const backgroundColor = isDarkMode ? "#333" : "#f5f5f5";
    const headerBackgroundColor = isDarkMode ? "#222" : "#e0e0e0";
    const jerseyItemBackgroundColor = isDarkMode
      ? "rgba(50,50,50,0.6)"
      : "rgba(220,220,220,0.6)";
      ...
      >
        ...
        >
          ...
          <View style={styles.seasonPickerContainer}>
            ...
            >
              <Text style={styles.seasonSelectorText}>{selectedSeason}</Text>
  ...
  const renderContent = () => {
    ...
    if (selectedTeam) {
      // Si estamos viendo estadísticas del equipo
      if (statisticsView) {
        return (
          <TeamStatisticsScreen
            team={selectedTeam}
            onBack={() => setStatisticsView(false)}
          />
        );
      }
